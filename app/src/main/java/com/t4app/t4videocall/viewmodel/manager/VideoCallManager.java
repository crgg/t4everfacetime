package com.t4app.t4videocall.viewmodel.manager;

import static com.t4app.t4videocall.VideoService.startService;
import static com.t4app.t4videocall.retrofit.ApiConfig.WEB_SOCKET_URL;
...
import com.t4app.t4videocall.viewmodel.RoomRepository;
import com.t4app.t4videocall.viewmodel.SfuWebSocketClient;
import com.t4app.t4videocall.events.VideoCallEvent;
import com.t4app.t4videocall.viewmodel.VideoCallEventListener;
...
public class VideoCallManager implements SfuWebSocketClient.Callback {
    ...
    public VideoCallManager(@ApplicationContext Context context) {
        ...
        this.signalingManager = new SignalingManager(sfuClient, roomState);
    }
    ...
    public void handleWebSocketMessage(String text) {
        try {
            ...
            if (room != null && !room.equals(roomState.getRoomName())) {
                return;
            }
    ...
    public void handleUserJoined(JSONObject json){
        try {
            ...
            if (remoteRoom.equalsIgnoreCase(roomState.getRoomName()) &&
                    !remoteUsername.equalsIgnoreCase(roomState.getUserName())){
                if (iAmCaller){
                ...
                sendVideoCallEvent(new VideoCallEvent.RemoteParticipantEvent.UserJoined(remoteUsername, remoteRoom));
            }
        }catch (Exception e){
    ...
    public void handleUserLeave(JSONObject json){
        try {
            ...
            if (remoteRoom.equalsIgnoreCase(roomState.getRoomName()) &&
                    !remoteUsername.equalsIgnoreCase(roomState.getUserName())){
                sendVideoCallEvent(new VideoCallEvent.RemoteParticipantEvent.RemoteUserLeave(remoteUsername, remoteRoom));
//                leaveRoom(sessionState);
            }
        }catch (Exception e){
            Log.e(TAG, "handleUserJoined: ", e);
        }
    ...
    public void handleToggleAudio(JSONObject json) {
        try {
            ...
            if (roomState.getRoomName() == null && roomState.getUserName() == null) {
                localParticipantManager.toggleAudio();

            } else if (roomState.getRoomName().isEmpty() && roomState.getUserName().isEmpty()) {
                localParticipantManager.toggleAudio();

            } else if (remoteUsername.equalsIgnoreCase(roomState.getUserName())) {
                localParticipantManager.toggleAudio();

            } else {
                sendVideoCallEvent(new VideoCallEvent.RemoteParticipantEvent.ToggleRemoteAudio(audioStatus));
            }
    ...
    public void handleToggleVideo(JSONObject json) {
        try {
            ...
            if (roomState.getRoomName() == null && roomState.getUserName() == null) {
                ...
                sendVideoCallEvent(new VideoCallEvent.RemoteParticipantEvent.ToggleRemoteVideo(videoStatus));
            }
                    ...
                    public void onAddTrack(RtpReceiver rtpReceiver, MediaStream[] mediaStreams) {
                        for (MediaStream stream : mediaStreams) {
                            if (!stream.videoTracks.isEmpty()) {
                                sendVideoCallEvent(new VideoCallEvent.RemoteParticipantEvent.AddRemoteUser(stream.videoTracks.get(0)));
...
//            sendVideoCallEvent(new VideoCallEvent.AddLocalUser(streamIds));
    ...
    public void sendToggleAudioEvent() {
        ...
        if (sfuClient.isConnected()){
            ...
            try {
                ...
                json.put("room", roomState.getRoomName());
                json.put("userName", roomState.getUserName());
    ...
    public void onConnected() {
        Log.d(TAG, "ON CONNECTED SOCKET: ");

        signalingManager.sendJoin();

        if (roomState == null){
            VideoCallManager.this.roomState = new Room(Room.State.CONNECTED, roomState.getRoomName());
        }

        startService(context, roomState.getRoomName());

        sendVideoCallEvent(new VideoCallEvent.Connected(
                localRoomInfo,
                roomState.getUserName(),
                iAmCaller,
                isInRoom));
    ...
    public void onDisconnected() {
        ...
        if (roomState != null){
            roomState.setState(Room.State.DISCONNECTED);
            roomState.setRoomName("");
        }
    ...
    public void createRoom(Room callSessionState){
        roomRepository.createRoom(callSessionState.getRoomName(),callSessionState.getUserName() , new RoomRepository.RoomCallback() {
            @Override
            public void onSuccess(RoomResponse response) {
                if (response.isOk()) {
                    RoomInfo roomInfo = response.getRoom();

                    roomState.setRoomName(roomInfo.getName());
                    roomState.setUserName(callSessionState.getUserName());

                    boolean inRoom = true;

                    joinRoom(roomState, inRoom);
                }
            }
    ...
    public void joinRoom(Room callSessionState, boolean inRoom){
        sendVideoCallEvent(VideoCallEvent.Connecting.INSTANCE);
        roomRepository.joinRoom(callSessionState.getRoomName(), callSessionState.getUserName(), new RoomRepository.RoomCallback() {
            @Override
            public void onSuccess(RoomResponse response) {
                if (response.isOk()){
                    RoomInfo roomInfo = response.getRoom();
    ...
    public void leaveRoom(Room callSessionState){
        roomRepository.leaveRoom(callSessionState.getRoomName(), callSessionState.getUserName(), new RoomRepository.RoomCallback() {
            @Override
            public void onSuccess(RoomResponse response) {
